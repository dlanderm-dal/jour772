...
-   Unzip in Code\comp\_text_class.\
...
#ATTN ROB: What did I do wrong? To double check I divided the total
salary of males by the sample size, that number checked out but the same
didnt work for the females 63198034/1025=61,656.6185365854

#rsw comment: I don't understand your question. we'll discuss in class.
...
3: Number of rows cut with filter of greater that 500 #Rob I genuinely
do not understand what this question is asking. Good faith objection.
Are you saying you want me to go back and filter the salaries by 500
instead of 1000? Or does the 500 somehow refer to the number of rows?
What do you mean cut with?

#Rsw comment: correct answer below. If you have a question about the
assignment and the wording, please email me for clarification
...
#rsw comment: this code chunk is incorrect
...
#rsw comment: this code chunk is correct
NYT <- subset(MediaBucks, (COMPANY=="NewYorkTimes") | (COMPANY=="NYT"))

#rsw comment: this code chunk is incorrect
#number of entries matches total of both paper entries
NYTsalary <- NYT %>% 
  filter(Salary >= 1000)
```
...
#ok man, look, I dont know how to do this besides just LOOKING at the very small table and seeing the Executive Editor or a black male who makes $350k, not quite sure how to, via code, isolate that data.
```
...
BusinessSalary <- Business %>% 
  filter(Salary >= 1000)
summary(BusinessSalary$Salary)
BloombergBus <- subset(MediaBucks, (COMPANY=="Bloomberg") | (COMPANY=="BloombergNews"))
summary(BloombergBus$Salary)

#Which one is higher?
#Bloomberg is higher in all respects except the 3rd Quartile, meaning that only 25% of people at bloomberg make over 54K
```

#rsw comment: correct answers to the questions above
...
#well shoot* man (can I swear here? Will it bring down my grade?), wish I had read this before going down that "or" rabbit hole *facepalm*
```
...
1: Who is making \$230,504 as a reporter??? #A senior reporter from
propublica #worthit

#rsw comment: need to see the code that allowed you to determine this
answer
...
#why did you put the median in plural?
```

3: Find highest paid editor. Resent them. Editorial director at Conde
Naste. I save my resentment for beneficiaries of the carried interest
loophole and executives of companies in defense, insurance , and asset
management

#rsw comment: need to see the code that allowed you to determine this
answer

4: Make a table for any position involving data #rsw comment: very
close. use JOB_DUTIES instead
...
#rsw comment: correct answers below Questions:
...
#refinement: ignore the title and remove nas and you find more editors
...
summary(Editors$Salary)
#top editor salary: $245,000

Editors %>% 
  filter(Salary ==245000)

#The highest paid editor is the editorial director at CondeNast who makes $245,000

#refinement: if you ignored the title and remove nas and you find more editors
# Editors <- subset(MediaBucks, grepl("?editor", TITLE, ignore.case = TRUE))
# summary(Editors$Salary, na.rm= TRUE)
