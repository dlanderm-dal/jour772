...
1. One or more parsing issues, call `problems()` on your data frame for details, e.g.:
  dat <- vroom(...)
  problems(dat)
2. When call problem I get a detailed list of all the problem rows with columns " row   col expected actual file "
...
**Task** Run the following codeblock to glimpse the data. What data type is the "Vendor Name" field? What data type is the "Amount" field? What data type is the "Date" column? Answer below. **Answer**character (chr)
...
**Task** Run the following codeblock to group by the vendor's zip code and show any that are less than five characters in length. How many zip codes do not have five characters? What do you think explains those records? **Answer**553 individual zip codes listed are not 5 characters in length although they each have their own count of frequency occuring in the dataset. These occur because zipcodes sometimes start with 1 or more 0s.
...
**Task** Run the following codeblock and describe below what each line is doing. Looking at the results, what jumps out at you as potentially problematic? **Answer**This code takes the md_grants_loans data frame, creates a pivot table of each category of each year and counts how many loans/grants/other categories were made that year, then organizes by fiscal year. The problem is there are other types listed in this sheet such as "contract" "L" and "NA." L is particularly troubling because it occurs 50 times in a year that already has loan data. Also 2009 has no loan data.
...
**Task** Run the following code to arrange the grants & loan data so that the smallest amount is first. Describe what you see. **Answer**Amounts ranging from $60 to $23480.22. Book says to notice there are only 2 grants below $100
...
**Task** Run the following codeblock to use the `clean_names()` function from janitor to standardize column names. How does it change the headers? Answer below. **Answer**spaces become underscores
...
**Task** Run the following codeblock to use str_to_upper() to make the source, grantee and description columns upper-cased. Describe what each line is doing. **Answer**Working on the cleaned_md_grants_loans data frame formed from md_grants_loans data frame. First taking all headers and making the spaces between words into underscores, then changing the category named grantor into "source," then taking the strings (characters in each cell below the following column names) in source, grantee, and description and captitalizing all the characters in the columns. Lastly displaying the dataframe.
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many duplicates are possible? **Answer**29 *possible* duplicate entries (58 rows are listed but they list both the theoretical original and the duplicate)
...
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? Answer below. **Answer**19453
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, creating a new column. Look at the difference in the result - what changed? **Answer**Created a new column called zip 5 that only contains the first 5 digits of the zip code (cutting off the 4 digit postal code)
